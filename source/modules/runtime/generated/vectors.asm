;
;	This file is automatically generated
;
	.section code
VectorTable:
	.word	LinkFloatAdd             ; $80 +
	.word	LinkFloatSubtract        ; $81 -
	.word	LinkFloatMultiply        ; $82 *
	.word	LinkFloatDivide          ; $83 /
	.word	LinkFloatPower           ; $84 ^
	.word	BinaryAnd                ; $85 and
	.word	BinaryOr                 ; $86 or
	.word	LinkCompareGreater       ; $87 >
	.word	LinkCompareEqual         ; $88 =
	.word	LinkCompareLess          ; $89 <
	.word	LinkCompareGreaterEqual  ; $8a >=
	.word	LinkCompareNotEqual      ; $8b <>
	.word	LinkCompareLessEqual     ; $8c <=
	.word	SignTOS                  ; $8d sgn
	.word	LinkFloatIntegerPartDown ; $8e int
	.word	AbsoluteTOS              ; $8f abs
	.word	UnaryUsr                 ; $90 usr
	.word	UnaryFre                 ; $91 fre
	.word	UnaryPos                 ; $92 pos
	.word	LinkFloatSquareRoot      ; $93 sqr
	.word	UnaryRND                 ; $94 rnd
	.word	LinkFloatLogarithm       ; $95 log
	.word	LinkFloatExponent        ; $96 exp
	.word	LinkFloatCosine          ; $97 cos
	.word	LinkFloatSine            ; $98 sin
	.word	LinkFloatTangent         ; $99 tan
	.word	LinkFloatArcTan          ; $9a atn
	.word	UnaryPeek                ; $9b peek
	.word	UnaryLen                 ; $9c len
	.word	Unary_Str                ; $9d str$
	.word	ValUnary                 ; $9e val
	.word	UnaryAsc                 ; $9f asc
	.word	UnaryChr                 ; $a0 chr$
	.word	Unary_Left               ; $a1 left$
	.word	Unary_Right              ; $a2 right$
	.word	Unary_Mid                ; $a3 mid$
	.word	CommandAssert            ; $a4 assert
	.word	UnaryBin                 ; $a5 bin$
	.word	PrintCharacter           ; $a6 print.chr
	.word	CompareStrings           ; $a7 s.cmp
	.word	CommandExit              ; $a8 exit
	.word	CommandDebug             ; $a9 debug
	.word	CommandFor               ; $aa for
	.word	CommandGet               ; $ab get
	.word	CommandReturn            ; $ac return
	.word	UnaryHex                 ; $ad hex$
	.word	LinkFloatCompare         ; $ae f.cmp
	.word	LinkDivideInt32          ; $af int.div
	.word	NegateTOS                ; $b0 negate
	.word	CommandNewLine           ; $b1 new.line
	.word	CommandNext              ; $b2 next
	.word	NotTOS                   ; $b3 not
	.word	CommandOn                ; $b4 on
	.word	CommandMoreOn            ; $b5 moreon
	.word	CommandPOKE              ; $b6 poke
	.word	GetChannel               ; $b7 getchannel
	.word	SetChannel               ; $b8 setchannel
	.word	PrintNumber              ; $b9 print.n
	.word	PrintString              ; $ba print.s
	.word	CommandRead              ; $bb read
	.word	CommandReadString        ; $bc read$
	.word	StringConcatenate        ; $bd concat
	.word	PrintTab                 ; $be print.tab
	.word	PrintPos                 ; $bf print.pos
	.word	PrintSpace               ; $c0 print.spc
	.word	TimeTOS                  ; $c1 ti
	.word	CommandShift             ; $c2 .shift
	.word	PushByteCommand          ; $c3 .byte
	.word	PushWordCommand          ; $c4 .word
	.word	CommandPushN             ; $c5 .float
	.word	CommandPushS             ; $c6 .string
	.word	CommandData              ; $c7 .data
	.word	CommandGoto              ; $c8 .goto
	.word	CommandGosub             ; $c9 .gosub
	.word	CommandGotoZ             ; $ca .goto.z
	.word	CommandGotoNZ            ; $cb .goto.nz
	.word	CommandVarSpace          ; $cc .varspace


ShiftVectorTable:
	.word	CommandClr               ; $c280 clr
	.word	CommandEnd               ; $c281 end
	.word	CommandRestore           ; $c282 restore
	.word	CommandStop              ; $c283 stop
	.word	CommandSYS               ; $c284 sys
	.word	CommandWAIT              ; $c285 wait
	.send code
